macro_rules! consts_impl {
    () => {
        impl<const N: usize> Decimal<N> {
            ...
            pub const NAN: Self = Self::new(UInt::ZERO, ControlBlock::NAN, Context::DEFAULT);

            /// Infinity (∞). More about [`±Infinity`](crate#special-values).
            pub const INFINITY: Self = Self::new(UInt::MAX, ControlBlock::INFINITY, Context::DEFAULT);

            /// Negative infinity (−∞). More about [`±Infinity`](crate#special-values).
            pub const NEG_INFINITY: Self = Self::new(UInt::MAX, ControlBlock::NEG_INFINITY, Context::DEFAULT);

            /// The smallest value that can be represented by this decimal type - (2<sup>N</sup> - 1) × 10<sup>32'768</sup>.
            pub const MIN: Self = Self::new(UInt::MAX, ControlBlock::basic(i16::MIN, Sign::Minus), Context::DEFAULT);

            /// The maximum value that this type can represent (2<sup>N</sup> - 1) × 10<sup>32'768</sup>.
            pub const MAX: Self = Self::new(UInt::MAX, ControlBlock::basic(i16::MIN, Sign::Plus), Context::DEFAULT);

            /// The smallest positive, normalized value that this type can represent.
            pub const MIN_POSITIVE: Self = Self::new(UInt::ONE, ControlBlock::basic(i16::MAX, Sign::Plus), Context::DEFAULT);
            ...
            pub const EPSILON: Self = Self::new(UInt::ONE, ControlBlock::basic(Intrinsics::<N>::MAX_CLENGTH as i16 - 1, Sign::Plus), Context::DEFAULT);
            ...
            pub const HALF: Self = Self::new(UInt::from_digit(5), ControlBlock::basic(1, Sign::Plus), Context::DEFAULT);
    ...
    (CONSTS $($name: ident $num: literal), *) => {
        $(
            ...
            pub const $name: Self = Self::new(UInt::$name, ControlBlock::basic(0, Sign::Plus), Context::DEFAULT);
        )*
    }
}
